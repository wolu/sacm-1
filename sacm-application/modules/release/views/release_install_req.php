<table id="dg" class="easyui-datagrid" title="Install & Configuration Request" style="height:500px;"
            url="<?=base_url('release/getJsonReq?status=OC')?>"
    		...
    		$.messager.confirm('Confirm', 'Status Akan di Update  ?', function(r) {
    			if (r){
    				$.post('<?=base_url('release/updateStatusReq')?>',{
    					id: row.NomorRequest, KodeStatus: 'IC'
    				}, function(result){
    					if (result.success) {
    					    $.messager.alert('Berhasil',result.success, 'info');
    						$('#dg').datagrid({ url: '<?=base_url('release/getJsonReq?status=OC') ?>'}, 'reload'); // reload the user data
    					}else{
