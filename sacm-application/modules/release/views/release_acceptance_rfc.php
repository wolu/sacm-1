<table id="dgrfc" class="easyui-datagrid" title="Acceptance RFC"  style="height:500px;"
            url="<?=base_url('release/getJsonRfc?status=IC')?>"
    		...
    		$.messager.confirm('Confirm', 'Status Akan di Update  ?', function(r) {
    			if (r){
    				$.post('<?= base_url('release/updateStatusRfc') ?>',{
    					id: row.NomorRFC, KodeStatus: 'RA'
    				}, function(result){
    					if (result.success) {
    					    $.messager.alert('Berhasil',result.success, 'info');
    						$('#dgrfc').datagrid({ url: '<?=base_url('release/getJsonRfc?status=IC') ?>'}, 'reload'); // reload the user data
    					}else{
