<table id="dgrfc" class="easyui-datagrid" title="Install & Configuration RFC"  style="height:500px;"
            url="<?=base_url('release/getJsonRfc?status=AT')?>"
    		...
    		$.messager.confirm('Confirm', 'Status Akan di Update  ?', function(r) {
    			if (r){
    				$.post('<?= base_url('release/updateStatusRfc')?>',{
    					id: row.NomorRFC, KodeStatus: 'IC'
    				}, function(result){
    					if (result.success) {
    					    $.messager.alert('Berhasil',result.success, 'info');
    						$('#dgrfc').datagrid({ url: '<?= base_url('release/getJsonRfc?status=AT') ?>'}, 'reload'); // reload the user data
    					}else{
