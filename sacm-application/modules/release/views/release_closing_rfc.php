<table id="dgrfc" class="easyui-datagrid" title="Closing RFC"  style="height:500px;"
            url="<?=base_url('release/getJsonRfc?status=PB')?>"
    			...
    			if (r){
    				$.post('<?= base_url('release/updateStatusRfc') ?>',{
    					...
    					if (result.success) {
    						...
    						$('#dgrfc').datagrid({ url: '<?= base_url('release/getJsonRfc?status=PB') ?>'}, 'reload'); // reload the user data
    					}else{
